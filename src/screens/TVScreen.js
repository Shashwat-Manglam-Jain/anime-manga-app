...
import { getTrendingTV, getPopularTV, getTopRatedTV, discoverTV, img } from "../api/tmdb";
import { TV_GENRES, RADIUS, SPACING } from "../utils/theme";
...
const TVCard = memo(function TVCard({ item, onPress, colors }) {
  return (
...
export default function TVScreen({ navigation }) {
  ...
  const fetchData = useCallback(async (t, p, g) => {
    ...
    try {
      let result;
      if (g) {
        result = await discoverTV(p, {
          with_genres: String(g),
          sort_by: t === "top_rated" ? "vote_average.desc" : "popularity.desc",
          "vote_count.gte": t === "top_rated" ? 100 : undefined,
        });
      } else if (t === "trending") {
        result = await getTrendingTV();
      } else if (t === "popular") {
        result = await getPopularTV(p);
      } else {
        result = await getTopRatedTV(p);
      }
      const items = mapShows(result.results);
  ...
  return (
    <ScreenWrapper>
      <Text style={[styles.heading, { color: colors.text }]}>TV Series</Text>
